...
10. **Group Image source** — new image uploader on the Group itself. Underlying field: resolved by 00-architecture.md Q8. Group has no photo column today; a new `Group.PhotoId` (nullable int → BinaryFile, mirroring `Person.PhotoId`) is added in Phase 2.
