...
| 04 | [04-phase-4-attributes.md](04-phase-4-attributes.md) | **locked** | 2026-05-09 | - | Attributes (Group + Member definitions). Drafted at end of Phase 3 session, locked 2026-05-09 with Q4.1 (one stack per category), Q4.2 (two Grids matching GroupTypeDetail's `groupMemberAttributes.partial.obs` pattern — read-only inherited Grid + editable custom Grid), Q4.3 (Section 5 reuses inherited `EntityBagBase.Attributes`/`AttributeValues`; Section 6 adds new `GroupMemberAttributes: PublicEditableAttributeBag[]` field), Q4.4 (Modal + AttributeEditor) all resolved. Implementation detail to settle at Phase 4 start: `GroupTypeOptionsBag.InheritedMemberAttributes` is currently `List<PublicAttributeBag>` but the canonical Grid needs `GroupTypeInheritedAttributeBag[]` (with `inheritedFromGroupTypeName`/`Url`); preferred resolution is to relocate that bag to a shared namespace and widen the field. |
