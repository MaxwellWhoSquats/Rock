...
| 06 | [06-phase-6-locations-editing.md](06-phase-6-locations-editing.md) | **locked** | 2026-05-12 | - | Locations editing modal + inline schedule logic (Section 4 Stack 2). Drafted at end of Phase 5; locked 2026-05-12 with all fourteen open questions resolved (Q6.1-Q6.14): flat dropdown initial-load (Q6.1), Name=empty inline-convention + reuse Schedule entity on Weekly↔Custom (Q6.2), server-side inactive-schedule reconciliation (Q6.3), preserve in-progress capacity edits via Vue reactivity (Q6.4), dedicated SaveGroupLocations helper inside WrapTransaction (Q6.5), Phase 6 owns DeleteInlineSchedule wiring (Q6.6), design-canonical modal title + Save button (Q6.7), capacity matrix gated on selectedSchedules.length > 0 with conditional well (Q6.8), single `selectedLocation` field + `selectedLocationMode` discriminator (Q6.9), Member tab visible with warning notification when source empty (Q6.10), FamilyMemberLocationOptions on GroupBag (Q6.11), duplicate detection on both Add and Edit (Q6.12), Edit-mode default tab inferred from groupMemberPersonAliasGuid (Q6.13), no reorder UI (Q6.14). |
