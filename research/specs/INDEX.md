...
| 04 | [04-phase-4-attributes.md](04-phase-4-attributes.md) | **completed (awaiting user commit)** | 2026-05-11 | - | Section 5 (Group Attribute Values: categorized stacks + `<AttributeValuesContainer>`) and Section 6 (Group Member Attribute Definitions: Inherited Grid + Custom Grid + Modal + AttributeEditor) shipped. `Save` WrapTransaction extended with step 4a (`entity.SaveAttributeValues`) + step 4b (member-attribute-defs sync via `Helper.SaveAttributeEdits`). `GroupTypeOptionsBag.InheritedMemberAttributes` widened to `GroupTypeInheritedAttributeBag[]` via cross-namespace import (MP-4.1; lighter touch than relocation). Section 6 bundled into a single new `groupMemberAttributes.partial.obs` matching the canonical GroupTypeDetail sibling shape (MP-4.2; deviation from spec V1/V2 split). `BuildInheritedMemberAttributes` walks `InheritedGroupTypeId` chain with circular-inheritance guard + URL template fallback. Phase 1 design item #17 (categories surfaced as stack headers) realized. Build clean. Two latent rows tracked → Phase 5: ADMINISTRATE / `AllowSpecificGroupMemberAttributes` panel gates on Section 6 + Add-path GroupType cascade refresh of `bag.attributes`. Awaiting user commit. |
